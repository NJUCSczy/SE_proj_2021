...
export function getStatusInfo(info,part=null){
    ...
    switch(stage){
        case 0:res=null;
        case '-1':res='状态错误';
        case '1':res='用户已提交申请，等待测试部审核';
        case '2':res='测试部驳回申请';
        case '3':res='测试部审核用户申请通过，等待市场部审核';
        case '4':res='市场部不受理用户申请';
        case '5':res='市场部对用户申请提出进一步审理';
        case '6':res='市场部确认受理用户请求，等待市场部提出议价';
    }
    if(res != '' && part === '软件委托测试合同')return '尚未创建'
    if(part === '软件项目委托测试申请书')return '市场部确认受理用户请求,软件项目委托测试申请书已完成'
    switch(stage){
        case '7':res='市场部提出议价，等待客户接受议价';
        case '8':res='客户不接受议价，委托终止';
        case '9':res='客户提出再议价';
        case '10':res='客户已接收议价，等到市场部填写受托人签章';
        case '11':res='市场部已填写受托人签章，等待客户填写委托人签章';
        case '12':res='测试合同已生成，等待测试部填写保密协议';
    }
    if(res != '' && part === '软件项目委托测试保密协议')return '尚未创建'
    if(part === '软件委托测试合同')return '测试合同已生成'
    switch(stage){
        case '13':res='测试部已填写保密协议，等待客户填写保密协议';
        case '14':res='用户已签署保密协议，委托完成';
    }
    if(part === '软件项目委托测试保密协议')return '用户已签署保密协议，委托完成'
    return res;
}